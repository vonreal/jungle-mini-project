...
def create_id():
    ...
    new_nickname_receive =request.form['new_nickname_give']
    ...
    #4 비밀번호를 해싱
    new_hash_passward = hashlib.sha256(new_password_receive.encode('utf-8')).hexdigest()

    #5 디비에 유저객체를 만들어 저장해
    doc = {
        'username': new_id_receive,
        'password': new_hash_passward,
...
def check_login():
    ...
    password_receive =request.form['passward_give']
    ...
    if results is not None:
        ...
        payload = {
            'id': id_receive,
            'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=60 * 60)  # 1시간 유효
        }
